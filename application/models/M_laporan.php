...
class M_laporan extends CI_Model {
	...
	function brg_keluar() {
		$this->db->select('tanggal,nama_brg,kategori,sub_1');
	...
	function get_brg_keluar_hari($where) {
		$this->db->select('tbl_so.tanggal,nama_brg,sum(jual) as jual, sum(beli) as beli, sum(beli-jual) as sisa,tbl_barang.kategori,tbl_barang.sub_1');
		$this->db->from('tbl_so');
	...
	function get_brg_keluar_minggu($where) {
		$this->db->select('tanggal,nama_brg,sum(jual) as jual, sum(beli) as beli, sum(beli-jual) as sisa,tbl_barang.kategori,tbl_barang.sub_1');
		$this->db->from('tbl_so');
		$this->db->join('tbl_barang','tbl_so.id_brg = tbl_barang.id');
		$this->db->where('tanggal>=',$where['tgl_1']);
		$this->db->where('tanggal<',$where['tgl_2']);
