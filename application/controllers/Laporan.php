...
class Laporan extends CI_Controller
{
	...
	function setting_excel($data)
	{ //SETTING EXCEL UNTUK LAPORAN STOCK OPNAME
		$heading 		= $data['judul'];
		$heading2 		= array('No', 'Tanggal', 'Nama Barang', 'Kategori', 'Sub 1', 'Stok Masuk', 'Stok Keluar', 'Stok Akhir');
		...
		foreach ($data['lap'] as $l) {
			...
			$this->excel->getActiveSheet()->setCellValue('f' . $row, $l->beli);
			$this->excel->getActiveSheet()->setCellValue('g' . $row, $l->jual);
			$this->excel->getActiveSheet()->setCellValue('h' . $row, $l->sisa);
			$row++;
			$no++;
			$tot_beli += $l->beli;
			$tot_jual += $l->jual;
		...
		$this->excel->getActiveSheet()->setCellValue('f' . $c, $tot_beli);
		$this->excel->getActiveSheet()->setCellValue('g' . $c, $tot_jual);
		$this->excel->getActiveSheet()->setCellValue('h' . $c, $tot_sisa);
	}

	public function lap_brg_keluar_hari()
	{ // LAPORAN STOCK OPNAME BERDASARKAN HARI
		$where 			= date('Y-m-d', strtotime($this->input->post('tanggal', TRUE)));
		$data = array(
			'judul' 	=> "KARTU STOCK OPNAME TOKO TANGGAL $where",
	...
	public function lap_brg_keluar_minggu()
	{
		...
		$data = array(
			'judul' 	=> "KARTU STOCK OPNAME TOKO TANGGAL $where[tgl_1] - $where[tgl_2]",
	...
	public function lap_brg_keluar_bln()
	{
		...
		$monthName 	= date("F", mktime(0, 0, 0, $where, 10));
		$data 		= array(
			'judul' 	=> "KARTU STOCK OPNAME TOKO BULAN $monthName $thn",
